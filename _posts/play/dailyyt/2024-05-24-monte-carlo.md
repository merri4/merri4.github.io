...
2. $$A$$를 포함하는 면적 $$A_{total}$$을 설정
> 좌우로 $$l$$의 길이로 뻗어나가고, $$e^{-x^2}$$의 최대 높이가 1므로 높이는 1인 직사각형을 설정한다(더 높게 설정해도 무방). 따라서 직사각형의 넓이는 $$2l\times1=2l$$이 된다. 물론 $$e^{-x^2}$$는 좌우로 무한히 뻗어나가는 모양이므로 면적 $$A$$를 완벽하게 덮을 순 없으나, $$l$$을 늘릴수록 해석적 근사값에 가까워질 수 있다.

3. 전체 면적 내에서 랜덤한 점을 찍었을 때 그 점이 함수 아래 ($$A$$ 안에) 있을 확률 $$p_{A} = \frac{A}{A_{total}}$$
> 이항하면 $$p_{A} \times A_{total} = A$$가 된다. 따라서 확률에 $$2l$$을 곱해 $$A$$를 구한다.
